  ...
  try {
    ...
    const results: any = {
      ...
      healthSummary: {
        ...
        RED: 0,
        UNKNOWN: 0,
        INCOMPATIBLE_QUANTITY: 0,
      },
      incompatibleCards: [] as any[],
      cardsTable: [] as any[],
    };
    ...
    // 2. Remove obsolete / unapproved chains (YouTube Followers, Comments, etc.)
    ...
    `);
    results.cleanedOldChains = Number((delRes as any).rowCount || 0);

    // 3. Upsert Canonical Fulfillment Chains & Slots
    for (const [key, chainData] of Object.entries(CANONICAL_SUPPLIER_ROUTING)) {
      ...
      // 4. Overwrite plans with canonical IDs
    ...
    // 5. Refresh Rates from Peakerr (GET/services only)
    const refreshRes = await SupplierRateMonitorService.refreshRatesFromProvider();
    results.ratesRefreshed = refreshRes.updatedCount;

    // 6. Query DB snapshots
    ...
    results.snapshotsCount = Number(countRes.rows[0]?.cnt || 0);

    // 7. Audit & Validate All 66 Catalog Cards
    for (const pkg of CLOUTFLOW_CATALOG_PACKAGES) {
      ...
      // Check min/max compatibility on priority and fallback suppliers
      ...
      if (prioritySnapshot || fallback1Snapshot) {
        if (priorityApproved && priorityQtyOk) {
          ...
          // If priority had incompatible quantity, but fallback is safe and compatible, it's GREEN
          health = 'GREEN';
        } else {
          health = 'RED';
        }
      ...
      if (isPriorityIncompatibleQuantity && !fb1QtyOk && !fb2QtyOk) {
        results.healthSummary.INCOMPATIBLE_QUANTITY++;
        results.incompatibleCards.push({
      ...
      results.healthSummary[health]++;

      results.cardsTable.push({
        platform: pkg.platform,
        service: pkg.service,
        plan: pkg.name,
        quantity: pkg.quantity,
    ...
    console.error('[CanonicalFixAPI] Error:', error);
    return NextResponse.json(
      {
        success: false,
        error: error.message || 'Canonical fix failed',
      },
      { status: 500 }
    );
  }
