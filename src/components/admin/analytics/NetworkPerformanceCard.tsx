  ...
  return (
    <div className="bg-white border border-[#D9E2E3] rounded-[10px] p-4 sm:p-5 shadow-[0_1px_2px_rgba(10,35,42,0.03),0_4px_12px_rgba(10,35,42,0.02)]">
      <div className="pb-3 sm:pb-4 border-b border-[#EEF2F3]">
        <h2 className="text-[15px] sm:text-[16px] font-bold text-[#142126] tracking-tight">Performance by Network</h2>
      ...
      <div data-testid="network-performance-mobile" className="md:hidden space-y-2.5 mt-3">
        {networks.length === 0 ? (
          <div className="py-6 text-center text-xs text-[#8A979D]">No network data recorded</div>
        ) : (
          networks.map((net) => (
            <div
              key={net.platformKey}
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <PlatformIcon platform={net.platformKey as Platform} size={20} />
                  <span className="font-bold text-[13px] text-[#142126]">{net.network}</span>
                </div>
              ...
              <div className="grid grid-cols-3 gap-2 pt-1 border-t border-[#EEF2F3] text-center text-[11px]">
                <div>
                  ...
                  <span className="font-bold text-[#142126] mt-0.5 block">{net.paidOrders.toLocaleString()}</span>
                </div>
                <div>
                  <span className="text-[#8A979D] uppercase text-[9.5px] font-semibold block">Units Sold</span>
                  <span className="font-medium text-[#65737A] mt-0.5 block">{net.quantitySold.toLocaleString()}</span>
                </div>
                <div>
                  <span className="text-[#8A979D] uppercase text-[9.5px] font-semibold block">AOV</span>
                  <span className="font-bold text-[#142126] mt-0.5 block">${net.aov.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                </div>
              </div>
            </div>
          ))
