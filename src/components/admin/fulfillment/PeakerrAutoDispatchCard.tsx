              ...
              <>
                  ...
                  {preCheckLoading ? (
                    ...
                    <div className="space-y-2 text-xs font-mono">
                      <div className={`p-2.5 rounded-lg border ${
                        ...
                        <p className="text-[10px] mt-1 text-neutral-300 font-sans">
                          {preCheckData.recoverySafety === 'BLOCKED_KNOWN_ACTIVE_ORDER'
                      ...
                      <div className="grid grid-cols-3 gap-2 text-[11px]">
                        <div className="p-2 bg-neutral-950 rounded border border-neutral-800">
                          <span className="text-neutral-500 block text-[9px]">Related Orders</span>
                          <strong className="text-white">{preCheckData.relatedOrders?.length || 0}</strong>
                        </div>
                        <div className="p-2 bg-neutral-950 rounded border border-neutral-800">
                          <span className="text-neutral-500 block text-[9px]">Known Active</span>
                          <strong className={preCheckData.activeOrders?.length > 0 ? "text-red-400" : "text-emerald-400"}>
                            {preCheckData.activeOrders?.length || 0}
                          </strong>
                        </div>
                        <div className="p-2 bg-neutral-950 rounded border border-neutral-800">
                          <span className="text-neutral-500 block text-[9px]">Completed/Terminal</span>
                          <strong className="text-neutral-300">{preCheckData.terminalOrders?.length || 0}</strong>
                        </div>
                      </div>

                      {preCheckData.activeOrders?.length > 0 && (
                        <div className="p-2 bg-neutral-950 rounded border border-red-500/20 text-[10px] space-y-1">
                          <span className="text-red-400 font-bold block">Active Orders List:</span>
                          {preCheckData.activeOrders.map((ao: any) => (
                            <div key={ao.id} className="text-neutral-300">
                              • Order: <strong>{ao.publicId}</strong> | Provider ID: <strong>#{ao.providerOrderId}</strong> | Status: <strong className="text-blue-400">{ao.livePeakerrStatus}</strong>
                            </div>
                          ))}
                        </div>
                      )}
                ...
                <div className="p-3 rounded-xl bg-red-500/10 border border-red-500/20 text-red-300 text-xs space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-1.5 bg-neutral-950 border border-neutral-700 rounded-lg text-white font-mono text-xs focus:outline-none focus:border-emerald-500 disabled:opacity-50"
                  />
